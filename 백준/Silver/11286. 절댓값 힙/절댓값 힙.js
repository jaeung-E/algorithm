...
function solution() {
  ...
  inputs.forEach((n) => {
    ...
    if (number === 0) answer.push(heap.poll()[1]);
    else heap.push([Math.abs(number), number]);
  });
...
class Heap {
  ...
  push(value) {
    ...
    while (
      (valueIndex > 1 &&
        this.heap[parentIndex][0] === this.heap[valueIndex][0] &&
        this.heap[parentIndex][1] > this.heap[valueIndex][1]) ||
      (valueIndex > 1 && this.heap[parentIndex][0] > this.heap[valueIndex][0])
    ) {
  ...
  isLower(a, b) {
    return (
      (this.heap[a] && this.heap[a][0] < this.heap[b][0]) ||
      (this.heap[a] &&
        this.heap[a][0] === this.heap[b][0] &&
        this.heap[a][1] < this.heap[b][1])
    );
  }
}
